 ...
 * UW_TN Fetcher
 * 
 * Handles fetching Translation Notes from Door43 using tarball archives
 */
...
import { UW_TN_Config } from './index';
...
export class UW_TN_Fetcher {
   ...
   */
  async fetchMetadata(config: UW_TN_Config): Promise<Door43ResourceMetadata> {
    const resourceConfig = {
      owner: config.owner,
      language: config.language,
      resourceId: 'tn', // Door43 resource ID
   ...
   */
  async fetchResource(config: UW_TN_Config): Promise<FetchedResource> {
    // Get metadata
    ...
    // Get file pattern
    const filePattern = this.server.getFilePattern('tn');
   ...
   */
  private async downloadAndExtractArchive(archiveUrl: string, config: UW_TN_Config): Promise<Map<string, string>> {
    console.log(`📦 Downloading TN archive: ${archiveUrl}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      extract.on('entry', (entry: any) => {
        ...
        // Filter for TSV files only (Translation Notes)
        if (filePath.endsWith('.tsv') && !filePath.includes('/.git/')) {
          let content = '';
      ...
      extract.on('end', () => {
        console.log(`📦 Extracted ${extractedFiles.size} TSV files`);
   ...
   */
  async downloadArchive(config: UW_TN_Config, format: 'tarball' | 'zipball' = 'tarball'): Promise<Buffer> {
    const metadata = await this.fetchMetadata(config);
   ...
   */
  async getResourceStats(config: UW_TN_Config): Promise<{
    totalFiles: number;
    totalSize: number;
    books: string[];
    lastUpdated: string;
   ...
   */
  async isAvailable(config: UW_TN_Config): Promise<boolean> {
    try {
   ...
   */
  async getAvailableVersions(config: UW_TN_Config): Promise<string[]> {
    const metadata = await this.fetchMetadata(config);
