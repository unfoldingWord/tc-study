...
import { Trash2, Book, FileText, HelpCircle, BookOpen, Languages, Building2 } from 'lucide-react'
import type { Door43Resource } from '@/types/manifest'

export function PackageResourceManager() {
      ...
      <div className="space-y-6">
        {Object.entries(groupedResources)
          .sort(([a], [b]) => {
            ...
            if (a === 'Aligned Bible') return -1
            if (b === 'Aligned Bible') return 1
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors flex-shrink-0 ml-3"
        ...
        <div className="text-sm text-gray-600">
          ...
          <div className="flex flex-wrap gap-2">
            {Array.from(languages).map(lang => (
              <span key={lang} className="px-2 py-1 bg-white border border-gray-300 rounded text-xs">
          ...
          <div className="flex flex-wrap gap-2">
            {Array.from(organizations).map(org => (
              <span key={org} className="px-2 py-1 bg-white border border-gray-300 rounded text-xs">
                {org}
              </span>
            ))}
          </div>
