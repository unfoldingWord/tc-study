...
export function PackageInfoForm() {
  const { manifest, setManifestField } = usePackageStore()
      ...
      <div className="max-w-2xl space-y-6">
        <div>
          ...
          <input
            type="text"
            value={manifest.name || ''}
            onChange={(e) => setManifestField('name', e.target.value)}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
            placeholder="e.g., My Multilingual Study Package"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) => setManifestField('description', e.target.value)}
            rows={3}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => setManifestField('version', e.target.value)}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
              placeholder="1.0.0"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setManifestField('metadata', { ...manifest.metadata, author: e.target.value })
              }
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
        ...
        <div>
          ...
          <select
            value={manifest.metadata?.license || 'CC BY-SA 4.0'}
            onChange={(e) =>
              setManifestField('metadata', { ...manifest.metadata, license: e.target.value })
            }
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
          >
