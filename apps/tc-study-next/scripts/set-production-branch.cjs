 ...
 * Uses wrangler's OAuth token from config. Run from apps/tc-study-next.
 *
...
const PROJECT_NAME = 'tc-study-next'
...
const req = https.request(options, (res) => {
  ...
  res.on('end', () => {
    ...
    if (json.success) {
      console.log(`Production branch set to "${PRODUCTION_BRANCH}" for tc-study-next.pages.dev`)
    } else {
