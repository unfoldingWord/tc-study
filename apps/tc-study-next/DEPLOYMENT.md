# TC-Study Next Deployment Guide

> Experimental fork. **Never** deploy this app to the `tc-study` Cloudflare Pages
> project (`tc-study.pages.dev`). Use project `tc-study-next`. Production GitHub
> Actions (`.github/workflows/deploy-tc-study.yml`) still deploys `apps/tc-study` only.
...
1. **Via Dashboard** (Recommended for first deployment):
   ...
   # Deploy
   wrangler pages deploy dist --project-name=tc-study-next
   ```
...
2. **Via Cloudflare Dashboard**:
   ...
   - Configure build settings:
     - **Build command**: `cd apps/tc-study-next && bun run build`
     - **Build output directory**: `apps/tc-study-next/dist`
     - **Root directory**: `/`
...
```bash
wrangler pages deployment list --project-name=tc-study-next
```
