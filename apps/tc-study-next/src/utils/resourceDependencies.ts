...
export async function checkDependenciesReady(
  ...
  catalogManager: {
    ...
    getResourceMetadata: (key: string) => Promise<{
      type?: string
      contentMetadata?: { ingredients?: unknown[] }
    } | null>
  },
  _debug: boolean = true
): Promise<boolean> {
  ...
  for (const dep of resourceType.dependencies) {
    ...
    for (const resourceKey of allResourceKeys) {
      ...
      const [resOwner, resLanguage, _resId] = parts
...
export function getRequiredDependencyResources(
  ...
  for (const dep of resourceType.dependencies) {
    ...
    const matchingResource = Array.from(availableForSelection.entries()).find(([_key, resource]) => {
