/**
 * Read V1 backup URL template:
 *   /read-v1/{lang}/{resource_type}/{nav_type}/{nav_ref}
 *
 * Also accepts book-only / incomplete tails that complete to a default start:
 *   /read-v1/{lang}/bible/{book}              → first ch:v (or first section) for current nav mode
 *   /read-v1/{lang}/bible/{nav_type}/{book}   → first ch:v (or first section) for that nav mode
 *   /read-v1/{lang}/obs/{storyNum}            → story N (story mode)
 *   /read-v1/{lang}/obs/ref/{storyNum}        → story N frame 1
 *
...
export const READ_V1_BASE = '/read-v1'
...
export type ReadRouteTail = {
   ...
   * (used for `/read-v1/{lang}/bible/{book}` book-only deep links).
   */
  navType?: string
 ...
 * e.g. `/read-v1/en/obs` → `{ resourceType: 'obs' }`
 * e.g. `/read-v1/en/obs/story` → `{ resourceType: 'obs', navType: 'story' }`
 */
...
export function buildReadPath(lang: string, tail: ReadRouteTail): string {
  ...
  return `${READ_V1_BASE}/${l}/${tail.resourceType}/${navType}/${ref}`
}
