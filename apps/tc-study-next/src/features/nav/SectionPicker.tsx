      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        {sections.length === 0 ? (
          <div className="flex-1 flex items-center justify-center">
            <AlertCircle className="w-12 h-12 text-fg-muted" />
          </div>
        ) : (
                  ...
                  >
                    <div
                      className={`
                        ...
                        ${isPicked ? 'bg-accent text-white' : 'bg-muted text-fg-secondary'}
                      `}
                    >
                      {idx + 1}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-medium text-fg truncate">{`Section ${idx + 1}`}</div>
                      <div className="text-sm text-fg-secondary mt-0.5 font-medium">
                        {startRef} - {endRef}
                      </div>
                    </div>

                    {isPicked && <Check className="w-5 h-5 text-accent flex-shrink-0" />}
                  </button>
                )
              })}
            </div>
