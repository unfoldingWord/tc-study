...
export function ManageCollectionDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay backdrop-blur-sm">
      <div className="relative w-full max-w-3xl max-h-[90vh] bg-surface border border-border rounded-2xl shadow-xl flex flex-col">
        {/* Header */}
        <div className="flex items-start gap-4 px-6 py-4 border-b border-border">
          <div className="flex-1 space-y-3">
            <input
              ...
              className="w-full px-3 py-2 text-lg font-semibold text-fg border-2 border-transparent hover:border-border focus:border-accent rounded-lg transition-colors outline-none"
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-3 py-2 text-sm text-fg-secondary border-2 border-transparent hover:border-border focus:border-accent rounded-lg resize-none transition-colors outline-none"
              aria-label="Collection description"
              title="Edit collection description"
            />
            <p className="text-sm text-fg-secondary px-3">
            ...
            <div className="space-y-2">
              {resources.map((resource: ResourceInfo | PackageResource, index: number) => {
                ...
                return (
                  <div
                    key={index}
                    className="group flex items-center justify-between p-3 bg-muted hover:bg-muted rounded-lg transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-fg">{resId}</span>
                          <span className="px-2 py-0.5 bg-muted text-fg-secondary text-xs rounded">
                            {langCode}
                          </span>
                        </div>
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-2 hover:bg-danger-soft rounded-lg transition-all"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
              className="p-2 text-fg-secondary hover:bg-muted rounded-lg transition-colors"
              title="Cancel"
