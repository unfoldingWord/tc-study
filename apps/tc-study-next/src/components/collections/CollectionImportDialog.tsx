    ...
    <div className="fixed inset-0 bg-overlay flex items-center justify-center z-50">
      <div className="bg-surface border border-border rounded-lg shadow-xl w-full max-w-md mx-4 max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center gap-3">
            <span title="Load collection">
              <Upload className="w-6 h-6 text-accent" aria-label="Load collection" />
            </span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {downloadProgress && (
            <div className="bg-accent-soft border border-accent rounded-lg p-4 space-y-3">
              ...
              <div className="text-xs text-accent-fg truncate font-mono bg-accent-soft px-2 py-1 rounded">
          ...
          {success && (
            <div className="bg-accent-soft border border-border rounded-lg p-3 flex items-center justify-center gap-2 text-accent-fg">
              <span title="Success">
    ...
    <div className="flex border-b">
      <button
        ...
        className={`flex-1 flex items-center justify-center gap-2 p-3 transition-colors ${
          mode === 'from-db'
            ? 'border-b-2 border-accent bg-accent-soft text-accent-fg'
      ...
      {packages.map((pkg) => (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            {selectedPackage?.id === pkg.id && <Check className="w-5 h-5 text-accent" />}
          </div>
        </button>
      ))}
    </>
  ...
  return (
    ...
    >
      <span title="Collection file">
        <FileArchive
          className={`w-16 h-16 mx-auto mb-3 ${file ? 'text-accent' : 'text-fg-muted'}`}
        />
      </span>
      {file ? (
        <>
          <p className="text-sm font-medium text-fg-secondary mb-1 truncate px-2">{file.name}</p>
