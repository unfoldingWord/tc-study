...
Default build = USJ-only (Helps tip `7e8159d`; Pipeline delete WIP when present).
...
| 9 | Rollback `VITE_USE_USJ_PIPELINE=0` | Legacy path sample | **N/A** | Rollback removed in Pipeline WIP; obsolete after delete |
...
| 2026-08-12 | QA | Helps `7e8159d` (+ local Pipeline delete WIP) | Unit **40/40**; Journey 4/8 **4/4**; Helps on `loadViewModel` / zero usfm-processor in helps path. | **DELETE incomplete on remote** — Pipeline must push usfm-processor/usfm-js removal; Platform CI `@usfm-tools` packaging |
