...
- **Helps cards:** idle `surface` + border; quote chips use `chip-quote*`; selected TN/TWL use soft blue `bg-accent-soft` + even `border-border` (no left bar / ring / purple or yellow wash). Prose links per [THEME.md](./THEME.md) (accent / fg-secondary — not rainbow hover chips).
