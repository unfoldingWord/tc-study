...
- **Helps cards:** idle `surface` + border; quote chips use `chip-quote*`; selected TN/TWL use `bg-muted/40` + even `border-border` (no left bar / ring / wash). Prose links per [THEME.md](./THEME.md) (accent / fg-secondary — not rainbow hover chips).
