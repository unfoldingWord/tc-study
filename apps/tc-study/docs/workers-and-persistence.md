...
| `prepared:` | `prepared:{typeId}:{resourceKey}:{book}:nav` · `prepared:{typeId}:{resourceKey}:{book}:{unit}:{tier}` | Versioned light / full / nav payload | `prepare.worker` and warm `prepare-unit` / `prepare-article` | ScriptureViewer, CombinedHelps, `useAlignedTokens` reconstruct, warm skip-if-exists | `SCRIPTURE_PREPARE_VERSION` / `NOTES_PREPARE_VERSION` (4) / `WORDS_LINKS_PREPARE_VERSION` (1). No TTL. |
| `helps-quote:` | `helps-quote:{helps}@{hs}:{ol}@{os}:{book}:{ch}` | `Record<linkId, CachedQuoteToken[]>` (empty array = settled miss) | Lane-1 `useQuoteTokens`; warm `quote-chapter` | Same + warm align (needs quotes first) | Version `3`. TTL **30 days** (`expiresAt`). |
| `helps-align:` | `helps-align:{helps}@{hs}:{ol}@{os}:{target}@{ts}:{book}:{ch}` | `Record<linkId, { p: number[]; m: 0\|1\|2 }>` | Lane-1 `useAlignedTokens`; warm `align-chapter` | `useAlignedTokens` reconstruct | Version `3`. TTL **30 days**. `m`: 0 miss, 1 semantic/zaln, 2 quote-text fallback. |
