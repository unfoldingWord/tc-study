...
describe('visible scripture highlight (no keep-alive / paint cache)', () => {
  ...
  test('Read/Studio mount one viewer; token-click always setHighlightTarget', () => {
    ...
    const content = readFileSync(join(import.meta.dir, 'components/ScriptureContent.tsx'), 'utf8')
    ...
    expect(highlighting).toContain('shouldNavigateScriptureToRef')
    expect(highlighting).toContain('navigateToReference')
    expect(highlighting).not.toContain('if (!applied.changed) return')
    expect(highlighting).not.toContain('applyHighlightSignal')
    expect(viewer).toContain('scriptureChapterTokensReady')
    expect(viewer).toContain('usjDisplayTokensReady')
    expect(content).toContain('currentRef.chapter')
    expect(content).toContain('[data-highlighted="true"]')
    expect(content).toContain('tokensReady')
  })
})
