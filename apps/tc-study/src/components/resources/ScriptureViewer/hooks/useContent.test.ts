...
describe('scripture content load retry (language-switch race)', () => {
  ...
  test('useContent hides previous viewModel as soon as resourceKey/loadKey changes', () => {
    ...
    expect(src).toContain('scriptureLane1Ready')
    expect(src).toContain('resolveLane1ScriptureViewModel')
    expect(src).toContain('includeRest: false')
    expect(src).not.toMatch(/lane1Ready:\s*!isLoading/)
    ...
    expect(src).not.toContain('prepViewModelBestEffort')
    expect(src).not.toContain('prepViewModelForRead')
    expect(src).not.toContain('attachFoldedMatchKeysToViewModel')
  })
  ...
  test('underline and highlight hooks fold locally (no session paint cache)', () => {
    ...
    expect(underlines).toContain('mapUnderlinesToIndices')
    ...
    expect(highlighting).toContain('rawSemanticIdForToken')
    expect(highlighting).toContain('handleInternedTokenClick')
