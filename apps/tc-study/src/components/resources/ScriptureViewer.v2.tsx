...
export function ScriptureViewer({
  ...
  useEffect(() => {
    ...
    const loadBookContent = async () => {
      console.log('📖 Loading content from catalog for book:', bookCode, 'resourceKey:', resourceKey)
      setIsLoading(true)
      setError(null)
      
      try {
        // Load scripture content from catalog (three-tier lookup)
        const content = await catalogManager.loadContent(resourceKey, bookCode)
        
        if (cancelled) return
        
        if (content) {
          // Content from ScriptureLoader should already be parsed
          // It should match our ParsedChapter[] format
          console.log('✅ Loaded content from catalog:', content)
          
          // For now, use the sample data format
          // TODO: ScriptureLoader should return ParsedChapter[] format
          const usfm = content as any
          if (typeof usfm === 'string') {
            const parsed = parseUSFM(usfm, bookCode)
            setLoadedContent(parsed)
            console.log('✅ Parsed', parsed.length, 'chapters for', bookCode)
          } else if (Array.isArray(usfm)) {
            // Already parsed
            setLoadedContent(usfm)
          } else {
            // Fall back to sample data
            console.warn('⚠️ Unexpected content format, using sample data')
            const sampleUsfm = SAMPLE_USFM_BY_BOOK[bookCode]
            if (sampleUsfm) {
              const parsed = parseUSFM(sampleUsfm, bookCode)
              setLoadedContent(parsed)
            } else {
          ...
          console.warn('⚠️ No content returned from catalog for:', bookCode)
          // Fall back to sample data
          const sampleUsfm = SAMPLE_USFM_BY_BOOK[bookCode]
          if (sampleUsfm) {
            const parsed = parseUSFM(sampleUsfm, bookCode)
        ...
        if (cancelled) return
        console.error('❌ Error loading content from catalog:', err)
  ...
  }, [currentRef.book, resourceKey, catalogManager])

  // Expose TOC on mount (if anchor resource)
  useEffect(() => {
    if (isAnchor) {
      const loadTOC = async () => {
        console.log('⚓ Loading TOC from catalog for resource:', resourceKey)
        
        try {
          // Get resource metadata to check if it has book list
          const metadata = await catalogManager.getResourceMetadata(resourceKey)
          
          if (metadata?.extra?.books) {
            // Use books from metadata if available
            const books: BookInfo[] = metadata.extra.books.map((book: any) => ({
              code: book.code,
            ...
            console.log('⚓ ScriptureViewer exposed TOC from metadata with', books.length, 'books')
          } else {
            // Fall back to sample data
            console.warn('⚠️ No book list in metadata, using sample data')
            ...
            app.setAnchorResource(resourceId, toc)
            console.log('⚓ ScriptureViewer exposed TOC from sample data with', books.length, 'books')
          }
        } catch (err) {
  ...
  }, [isAnchor, resourceId, resourceKey, catalogManager])
                    ...
                    return (
                      <span key={token.id}>
                        <span
                          ...
                          className={`
                            cursor-pointer px-1 py-0.5 rounded transition-all text-gray-900
