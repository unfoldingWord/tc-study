 ...
 */
async function* _readCacheEntriesInBatches(
  db: IDBDatabase,
  storeName: string,
  resourceKeys: string[],
  batchSize: number = 100
): AsyncGenerator<Array<{ key: string; entry: CacheEntry }>, void, unknown> {
  const tx = db.transaction(storeName, 'readonly')
  const store = tx.objectStore(storeName)

  let batch: Array<{ key: string; entry: CacheEntry }> = []
  let _totalRead = 0
  ...
  async function processBatch(_batchData: Array<{ key: string; entry: CacheEntry }>): Promise<void> {
...
async function exportCollection(config: ExportWorkerMessage['data']): Promise<void> {
  ...
  const resourceBuffers = new Map<string, Array<{ key: string; entry: CacheEntry }>>()
  ...
  const flushResourceBuffer = (resourceKey: string, buffer: Array<{ key: string; entry: CacheEntry }>) => {
  ...
  const _totalFiles = Array.from(resourceFileCounters.values()).reduce((sum, count) => sum + count, 0)
...
function generateReadme(
  manifest: ExportWorkerMessage['data']['collection'] & {
    createdAt?: string
...
**Created:** ${new Date(manifest.createdAt ?? Date.now()).toLocaleString()}
**Format:** BT Synergy Collection v${manifest.formatVersion ?? '1.0.0'}
...
### Panels (${manifest.panelLayout?.panels.length ?? 0})
