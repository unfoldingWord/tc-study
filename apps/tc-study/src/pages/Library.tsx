 ...
 * 
 * Shows all resources with their metadata, download status (online/offline),
 * and allows downloading/managing individual resources and books.
 */

import { useState, useEffect } from 'react'
import { useCatalogManager, useCompletenessChecker } from '../contexts/CatalogContext'
import { Library as LibraryIcon, Search, Eye, Code, RefreshCw, Download, Trash2, Plus, CheckCircle, Circle, Loader } from 'lucide-react'
...
export default function Library() {
  ...
  const [completenessStatus, setCompletenessStatus] = useState<Map<string, ResourceCompletenessStatus>>(new Map())
  ...
  const loadCatalog = async () => {
    ...
    try {
      ...
      // Check completeness for all resources using ResourceCompletenessChecker
      ...
      console.log('📊 Completeness check:', {
        ...
        complete: Array.from(statusMap.values()).filter(s => s.isComplete).length
      })
  ...
  // Poll for completeness updates every 3 seconds to detect background downloads
  useEffect(() => {
    const interval = setInterval(async () => {
      if (resources.length > 0) {
        // Re-check completeness for all resources
        const statusMap = new Map<string, ResourceCompletenessStatus>()
  ...
  const handleDownloadAll = async (resource: ResourceMetadata) => {
    ...
    try {
      ...
      await catalogManager.downloadResource(resourceKey, {}, (progress: any) => {
  ...
  /**
   * Check if a resource is fully downloaded (uses completeness checker)
   */
  ...
  /**
   * Check if an ingredient is downloaded
   * Note: This checks the cache for the specific ingredient
   */
  const isIngredientDownloaded = (resource: ResourceMetadata, ingredientId: string): boolean => {
    // If the whole resource is complete, all ingredients are downloaded
    const resourceKey = getResourceKey(resource)
    if (isResourceComplete(resourceKey)) {
      return true
    }
    
    // Otherwise check the old metadata (fallback for partial downloads)
          ...
          <div className="grid grid-cols-4 gap-6">
            ...
            <div className="text-center">
              <div className="text-3xl font-semibold text-blue-600">
                {Array.from(completenessStatus.values()).filter(s => s.isComplete).length} / {resources.length}
              </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        {/* Compact download status using ResourceCompletenessChecker */}
                        {(() => {
                          const status = completenessStatus.get(getResourceKey(resource))
                          ...
                          if (status.isComplete) {
                            ...
                            return <Circle className="w-4 h-4 text-red-500 flex-shrink-0" aria-label={`Error: ${status.error}`} />
                          } else {
                            ...
                            <summary className="cursor-pointer text-xs text-gray-500 hover:text-gray-900 transition-colors flex items-center gap-2">
                              ...
                              <span>
                                {(() => {
                                  ...
                                  const status = completenessStatus.get(resourceKey)
                                  if (status?.isComplete) {
                                    return `${resource.contentMetadata.ingredients.length}/${resource.contentMetadata.ingredients.length} downloaded`
                                  }
                                  const downloadedCount = resource.contentMetadata.ingredients.filter(ing => 
                                    isIngredientDownloaded(resource, ing.identifier)
                                  ).length
                                  return `${downloadedCount}/${resource.contentMetadata.ingredients.length} downloaded`
                                })()}
                              </span>
                            </summary>
                    ...
                    <div className="ml-4 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {/* Download All button - show if not complete */}
                      {(() => {
                        const status = completenessStatus.get(getResourceKey(resource))
