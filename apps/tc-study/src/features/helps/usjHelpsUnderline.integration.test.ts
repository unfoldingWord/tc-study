 ...
 *
 * USJ-only: viewModel → viewModelToOptimizedChapters / extractUsjBroadcastTokens.
 */
...
async function processUsjViewModel(usfm: string, bookId: string, language: string) {
...
describe('USJ CombinedHelps underline / alignment path (Titus)', () => {
  test('quote match Παῦλος → semantic ID underlines ULT Paul via alignedOriginalWordIds', async () => {
    ...
    const broadcast = extractUsjBroadcastTokens(ult, 1, 1, 1, 999)
  ...
  test('TN-style Θεοῦ occurrence 1 → God underline group', async () => {
    ...
    const broadcast = extractUsjBroadcastTokens(ult, 1, 1, 1, 20)
