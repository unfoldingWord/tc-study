 ...
 */
export abstract class BaseCatalogAdapter implements CatalogStorageAdapter {
  ...
  async getStats(): Promise<CatalogStats> {
    ...
    return {
      ...
      byFormat: {},
      availableOffline: 0,
      availableOnline: resources.length,
      bundledResources: 0,
  ...
  protected matchesFilters(resource: ResourceMetadata, filters: CatalogQuery): boolean {
    ...
    if (filters.hasBooks !== undefined) {
      const hasBooks = !!(resource as any).books && (resource as any).books.length > 0
      if (hasBooks !== filters.hasBooks) return false
    }
    
    // Has relations filter
    if (filters.hasRelations !== undefined) {
      const hasRelations = !!(resource as any).relations && (resource as any).relations.length > 0
